window.addEventListener("DOMContentLoaded", (event) => {
  ...
  const pasubur = document.getElementById("pasubur");
  if (pasubur) {
    new simpleDatatables.DataTable(pasubur, {
      // searchable: false,
      // paging: false,
      perPage: 25,
    });
  }
  const wasubur = document.getElementById("wasubur");
  if (wasubur) {
    new simpleDatatables.DataTable(wasubur, {
      // searchable: false,
      paging: false,
      // perPage: 50,
    });
